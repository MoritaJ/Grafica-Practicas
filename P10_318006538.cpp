...
float ciclo3 = 0, ciclo4 = 0, ciclo5 = 0, ciclo6 = 0;

//Nave
float movAla = 0.0f;
float movAlaOffset = 50.0f;
float rotAla = 0.0f;
float alaOffset = 2.5f;
int dirAla = 1;
...
int main()
{
	...
	Planeta_R = Model();
	Planeta_R.LoadModel("Models/Planeta_Rey.obj");
	...
	KeyFrame[3].movAvion_y = -3.0f;
	KeyFrame[3].giroAvion = 180.0f;
	...
	// Gira de regreso a orientación original
	...
	while (!mainWindow.getShouldClose())
	{
		...
		model = modelaux;
		model = glm::translate(model, glm::vec3(0.0f, 0.0f, 0.3f));
		model = glm::rotate(model,(270* toRadians)+ rotAla * toRadians, glm::vec3(0.0f, 1.0f, 0.0f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		Ala_M.RenderModel();
...
void inputKeyframes(bool* keys)
{
	...
	if (keys[GLFW_KEY_0])
	{
		if (habilitaranimacion < 1)  
		{
			...
			habilitaranimacion++;    // <- bloquea después de imprimir
			printf("Ya puedes reproducir de nuevo\n");
		}
	}
	...
	if (keys[GLFW_KEY_L])
	{
		...
		{
			saveFrame();
			printf("presiona P para guardar otro frame\n");
			guardoFrame++;
			reinicioFrame = 0;
		}
	}
	if (keys[GLFW_KEY_P])
	{
		...
		{
			guardoFrame = 0;
			printf("Ya puedes guardar otro frame con L\n");
			reinicioFrame++;  // <- faltaba esto
		}
	}
	if (keys[GLFW_KEY_1])
	{
		if (ciclo < 1)
		{
			...
			ciclo++;
			ciclo2 = 0;
			printf("Presiona 2 para habilitar la variable\n");
		}
	}
	if (keys[GLFW_KEY_2])
	{
		...
		{
			ciclo = 0;
			printf("Ya puedes modificar con tecla 1\n");
			ciclo2++;  // <- faltaba esto
		}
	}
	if (keys[GLFW_KEY_3])
	...
	if (keys[GLFW_KEY_4])
	{
		...
		{
			ciclo3 = 0;
			printf("Ya puedes modificar Y con tecla 3\n");
			ciclo4++;
		}
	}
	if (keys[GLFW_KEY_5])
	...
	if (keys[GLFW_KEY_6])
	{
		...
		{
			ciclo5 = 0;
			printf("Ya puedes modificar giro con tecla 5\n");
			ciclo6++;
		}
	}
}